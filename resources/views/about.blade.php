...
@section('content')
<div x-data="aboutPage()" class="space-y-10 sm:space-y-14 py-6 sm:py-12">
      ...
      <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 sm:gap-10 items-center relative z-10">
        ...
        <div class="lg:col-span-4 flex justify-center lg:justify-start">
          <div class="relative group">
            <div class="w-44 h-44 sm:w-64 sm:h-64 rounded-full bg-gradient-to-tr from-brand-card to-brand-cardLight border-4 border-brand-gold/50 overflow-hidden shadow-glow-gold flex items-center justify-center">
              <img
        ...
        <!-- Bio Text -->
        <div class="lg:col-span-8 space-y-4 sm:space-y-5 text-center" :class="lang === 'fa' ? 'lg:text-right' : 'lg:text-left'">
          <div>
            <h1 class="text-2xl sm:text-5xl font-black text-white font-heading leading-tight" x-text="t.teacherName[lang]"></h1>
            <p class="text-sm sm:text-lg text-brand-gold font-bold mt-1.5" x-text="t.teacherRole[lang]"></p>
          </div>
          
          <p class="text-brand-slate text-xs sm:text-base leading-relaxed max-w-3xl" x-text="t.teacherBioFull[lang]"></p>

          <!-- دکمه اختصاصی ارتباط مستقیم (جایگزین دانلود رزومه) -->
          <div class="pt-2 flex flex-wrap items-center justify-center lg:justify-start gap-4">
            <a
              ...
              class="bg-gradient-to-r from-brand-gold to-brand-goldHover text-brand-darkest px-7 py-3 rounded-xl text-xs sm:text-sm font-extrabold shadow-glow-gold transition-all active:scale-95 flex items-center gap-2"
            >
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-6">
      <template x-for="stat in stats" :key="stat.id">
        <div class="glass-card p-4 sm:p-6 rounded-2xl sm:rounded-3xl text-center space-y-1.5 border border-white/5 hover:border-brand-gold/30 transition-colors">
          <div class="text-2xl sm:text-4xl font-black text-gold-gradient" x-text="num(stat.value[lang])"></div>
          <div class="text-[11px] sm:text-sm font-bold text-brand-slate" x-text="stat.label[lang]"></div>
        </div>
      </template>
    </div>
  </section>
  ...
  <section class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
    <div class="glass-card rounded-2xl sm:rounded-3xl p-5 sm:p-10 border border-white/5 space-y-4 sm:space-y-6">
      <div class="flex items-center gap-2.5 border-b border-white/5 pb-3">
        <span class="w-1.5 h-6 bg-brand-gold rounded-full"></span>
        <h2 class="text-base sm:text-2xl font-black text-white font-heading" x-text="t.philosophyTitle[lang]"></h2>
      </div>
      
      <div class="space-y-3 text-brand-slate text-xs sm:text-base leading-relaxed sm:leading-loose" :class="lang === 'fa' ? 'text-justify' : 'text-left'">
  ...
  function aboutPage() {
    return {
      ...
      t: {
        ...
        contactMe: { fa: 'ارتباط مستقیم و مشاوره', en: 'Direct Contact' },
