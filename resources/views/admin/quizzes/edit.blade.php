        ...
        <div class="glass-card p-6 rounded-3xl border border-white/5 space-y-4">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div class="space-y-2">
                    ...
                    <input type="text" name="title_fa" value="{{ $quiz->title_fa }}" required class="w-full bg-brand-dark border border-white/10 rounded-xl px-4 py-2.5 text-xs text-white focus:outline-none focus:border-brand-gold" />
                </div>

                <div class="space-y-2">
                    <label class="text-xs text-brand-slate font-bold">English Quiz Title</label>
                    <input type="text" name="title_en" value="{{ $quiz->title_en }}" dir="ltr" required class="w-full bg-brand-dark border border-white/10 rounded-xl px-4 py-2.5 text-xs text-white focus:outline-none focus:border-brand-gold" />
                </div>
                ...
                <div class="sm:col-span-2 flex items-center gap-2 pt-2">
                    <input type="checkbox" name="is_published" id="is_published" value="1" {{ $quiz->is_published ? 'checked' : '' }} class="rounded border-white/10 text-brand-gold focus:ring-brand-gold">
                ...
                <div class="glass-card p-6 rounded-3xl border border-white/5 space-y-4">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div class="space-y-1">
                            ...
                            <input type="text" :name="'questions['+qIndex+'][text_en]'" x-model="q.text_en" dir="ltr" required class="w-full bg-brand-dark border border-white/10 rounded-xl px-4 py-2 text-xs text-white focus:outline-none focus:border-brand-gold" />
                        </div>
                        <div class="space-y-1">
                            <label class="text-[11px] text-brand-slate font-bold">صورت سوال به فارسی (اختیاری)</label>
                            <input type="text" :name="'questions['+qIndex+'][text_fa]'" x-model="q.text_fa" class="w-full bg-brand-dark border border-white/10 rounded-xl px-4 py-2 text-xs text-white focus:outline-none focus:border-brand-gold" />
                        </div>
                    </div>

                    <div class="space-y-2 pt-2">
                        <label class="text-[11px] text-brand-slate font-bold">گزینه‌ها (پاسخ صحیح را مشخص کنید):</label>
                        <div class="space-y-2">
                            <template x-for="(opt, oIndex) in q.options" :key="oIndex">
                                <div class="flex items-center gap-2 bg-brand-dark/60 p-2.5 rounded-xl border border-white/5">
                                    ...
                                    <input type="text" :name="'questions['+qIndex+'][options]['+oIndex+'][text_en]'" x-model="opt.text_en" dir="ltr" required class="w-1/2 bg-brand-dark border border-white/5 rounded-lg px-3 py-1.5 text-xs text-white focus:outline-none focus:border-brand-gold" />
                                    <input type="text" :name="'questions['+qIndex+'][options]['+oIndex+'][text_fa]'" x-model="opt.text_fa" class="w-1/2 bg-brand-dark border border-white/5 rounded-lg px-3 py-1.5 text-xs text-white focus:outline-none focus:border-brand-gold" />
                                </div>
                            </template>
                        </div>
                    </div>
